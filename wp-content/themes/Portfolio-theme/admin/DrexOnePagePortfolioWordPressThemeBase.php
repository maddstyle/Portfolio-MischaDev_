<?php
if(!class_exists("DrexOnePagePortfolioWordPressThemeBase")) {
	class DrexOnePagePortfolioWordPressThemeBase {
    	public $key = "6DE19C6FCA447E7D";
    	private $product_id = "9";
    	private $product_base = "drex";
    	private $server_host = "http://webredox.net/licence/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_drex_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=drex_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
