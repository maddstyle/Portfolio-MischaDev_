...
// $this->sections[] = array(
            ...
            //     'title'  => esc_html__( 'Demo Content', 'drex' ),
            //     'desc'   => esc_html__( 'Description Goes Here', 'drex' ),
...
if ( !function_exists( 'wbc_importer_description_text' ) ) {
	...
	function wbc_importer_description_text( $description ) {
		...
		$message = '<p>'. esc_html__( 'Best if used on new WordPress install.', 'drex' ) .'</p>';
		$message .= '<p>'. esc_html__( 'Images are for demo purpose only.', 'drex' ) .'</p>';
...
if ( !function_exists( 'wbc_extended_example' ) ) {
	function wbc_extended_example( $demo_active_import , $demo_directory_path ) {
		...
		*************************************************************************/
		...
		if ( class_exists( 'RevSlider' ) ) {
			...
			$wbc_sliders_array = array(
				'Revolution' => 'home-slider.zip', //Set slider zip name
		...
		*************************************************************************/
		...
		// If it's demo1 - demo6
		$wbc_menu_array = array( 'Image', 'Slider', 'Slider_thumbnails', 'Typed', 'Youtube', 'Vimeo' );

		if ( isset( $demo_active_import[$current_key]['directory'] ) && !empty( $demo_active_import[$current_key]['directory'] ) && in_array( $demo_active_import[$current_key]['directory'], $wbc_menu_array ) ) {
			
			$home_menu = get_term_by( 'name', 'One Page', 'nav_menu' );

			if ( isset( $home_menu->term_id ) ) {
				set_theme_mod( 'nav_menu_locations', array(
						'main-menu' => $home_menu->term_id,
